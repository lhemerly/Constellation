...
## 2024-06-25 - Prevent heap allocations in CacheMiddleware using fixed-size byte array
**Learning:** Instantiating `sha256.New()` and converting the hash to a hex string (`hex.EncodeToString`) dynamically allocates memory on the heap. By instead using `sha256.Sum256(input)` directly and using the resulting `[32]byte` fixed-size array as the map key, we completely eliminate these unnecessary allocations while generating identical cryptographic hash collision protection.
**Action:** When hashing byte slices to generate keys, avoid hex strings. Use `sha256.Sum256` and fixed array types like `[32]byte` as keys for maps directly to save memory and CPU cycles.
