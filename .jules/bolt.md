...
## 2024-05-19 - Optimize CacheMiddleware Hash Key Generation
**Learning:** Using `sha256.New()` followed by `hasher.Write()` and `hex.EncodeToString(hasher.Sum(nil))` for generating string map keys allocates objects on the heap (3 allocations/op) and consumes unnecessary CPU cycles (~540ns/op). The built-in `sha256.Sum256(input)` provides the hash directly as a `[32]byte` array, which can be used natively as a map key in Go without string conversion.
**Action:** When hashing byte slices to use as dictionary or map keys (e.g. for caching or deduplication), always prefer using `sha256.Sum256(input)` and a map keyed by `[32]byte` rather than converting to a hex string. This avoids heap allocations entirely (0 allocs) and yields a ~40% performance gain on the hot path.
