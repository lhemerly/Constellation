...
## 2024-05-23 - Optimize Lock Contention and Array Appends in MapReduceNode
**Learning:** In the `MapReduceNode.mapReduceProcess` function, using a `sync.Mutex` and `append` to collect results from concurrent mappers created lock contention and resulted in non-deterministic ordering. Furthermore, repeatedly using `append` to flatten the final results byte slice caused unnecessary memory reallocations.
**Action:** When aggregating results from multiple concurrent goroutines, pre-allocate the result slice based on the known count and assign values by index instead of using `sync.Mutex` and `append`. For concatenating multiple byte slices, always pre-calculate the total target capacity and initialize the resulting slice with `make([]byte, 0, totalLen)` to prevent repeated memory reallocations.
