...
## 2026-06-05 - Optimize CacheMiddleware Hash Key Allocation
**Learning:** In high-frequency hot paths like `CacheMiddleware`, creating a new SHA256 hasher (`sha256.New()`) and converting the result to a hex string (`hex.EncodeToString()`) causes unnecessary heap allocations, degrading performance and increasing GC pressure.
**Action:** Prefer using `sha256.Sum256(input)` which returns a `[32]byte` array directly. Use this array as the map key instead of converting it to a string. This eliminates allocations entirely (0 allocs vs 3 allocs per op) and can roughly double the execution speed of the cache key generation.
