...
## 2024-07-06 - Optimize CacheMiddleware Key Generation
**Learning:** Using `sha256.New()` and `hex.EncodeToString()` to generate string-based cache keys from byte slices causes significant unnecessary heap allocations (3 allocs/op) in the hot path.
**Action:** Replace dynamic hasher instantiation and string encoding with `sha256.Sum256(input)` directly, using the resulting fixed-size array `[32]byte` as the map key. This reduces memory allocations to 0 allocs/op and improves speed from ~521ns/op to ~299ns/op. Apply this pattern whenever computing cache keys for byte payloads in high-throughput paths.
