...
## 2024-05-18 - Optimize MapReduceNode Map/Reduce phase lock contention and allocation
**Learning:** In the `MapReduceNode`, iterating over concurrent results using `sync.Mutex` and `append(results, res)` causes high lock contention during the Map phase. In the Reduce phase, repeatedly appending `[][]byte` arrays to a `[]byte` array causes dynamic resizing and continuous memory allocations.
**Action:** When aggregating results from multiple goroutines concurrently, eliminate the lock contention by pre-allocating the results slice based on the number of workers, and assigning values by index (e.g. `results[i] = res`). For the reduction phase, pre-calculate the total combined byte length using a quick `for` loop, then allocate the target slice using `make([]byte, 0, totalLen)`.
