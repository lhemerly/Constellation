...
## 2024-05-22 - Optimize CacheMiddleware Hash Key Generation
**Learning:** Generating the SHA-256 hash in `CacheMiddleware` using `sha256.New()`, `Write()`, and `hex.EncodeToString(hasher.Sum(nil))` created multiple unnecessary heap allocations per cached request, negatively impacting memory footprint and latency.
**Action:** Replaced the allocation-heavy sequence with a direct `key := sha256.Sum256(input)` and changed the internal cache map signature to use `[32]byte` as the key type. Arrays are fully comparable in Go and avoid all the string encoding overhead. Apply this strategy in any cache or hashing logic occurring on hot paths.
