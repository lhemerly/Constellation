...
## 2024-06-04 - Optimize CacheMiddleware Hash Key Generation
**Learning:** Generating SHA256 hashes for caching using `sha256.New()`, `.Write()`, and `.Sum()`, and then encoding them to a string via `hex.EncodeToString` introduces significant overhead and memory allocation in hot paths like the `CacheMiddleware`.
**Action:** Replace this pattern with the direct `sha256.Sum256(input)` function, which is faster and allocates less. Furthermore, since `[32]byte` is a comparable type in Go, use it directly as the map key instead of stringifying it. This reduces allocation and CPU overhead per process request.
