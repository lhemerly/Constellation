...
## 2026-05-09 - Optimize Lock Contention and Memory Allocation in MapReduceNode
**Learning:** In concurrent fan-out operations like `MapReduceNode.mapReduceProcess`, using a `sync.Mutex` alongside dynamic `append()` to aggregate results from multiple mapper goroutines creates unnecessary lock contention. Additionally, when aggregating byte slices, repeating `append` for a flattened output triggers repeated memory reallocations.
**Action:** When aggregating results from multiple predictable, concurrent sources, always pre-allocate the result slices (`results := make([][]byte, numMappers)`) and use index-based assignment within the goroutine (`results[i] = res`) instead of appending inside a mutex lock. For flattening byte slices, calculate the total length first and allocate the final slice using `make([]byte, 0, totalLen)` to eliminate reallocation overhead.
