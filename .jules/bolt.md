...
**Learning:** Appending to a slice concurrently using a `sync.Mutex` inside the `mapReduceProcess` loop causes lock contention, slowing down parallel mapping. The benchmark without mutex is around 40% faster and reduces allocations significantly.
**Action:** When aggregating results from multiple concurrent goroutines where the final size is known (like `mr.mappers`), pre-allocate the result and error slices (`make([]T, len)`). Assign the values by index within the goroutines instead of locking and appending. Also, calculate the required capacity for flattened byte slices before combining to avoid multiple allocations during appends.
