...
## 2026-05-31 - Optimize MapReduceNode.mapReduceProcess
**Learning:** Using a `sync.Mutex` to aggregate results from concurrent mapper goroutines in `MapReduceNode` causes lock contention, and repeatedly appending to slices leads to memory reallocations, significantly slowing down the process. The benchmark showed ~10200 ns/op.
**Action:** Pre-allocate result and error slices based on the known number of mappers and assign values by index. Pre-calculate the total length of reduced output to pre-allocate capacity and avoid reallocation during concatenation. Benchmark improved to ~8400 ns/op (an 18% improvement). Ensure to pre-allocate fixed-size buffers and use index assignments instead of locks for fan-out aggregations.
