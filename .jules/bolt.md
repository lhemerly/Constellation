...
## 2024-05-24 - MapReduceNode Goroutine Aggregation Lock Contention
**Learning:** Using `sync.Mutex` and `append` to aggregate results from multiple goroutines inside a MapReduce phase creates significant lock contention, especially as the number of mappers increases. Furthermore, slice reallocation during the reduce phase causes unnecessary memory allocations.
**Action:** When aggregating results from a known number of concurrent goroutines, always pre-allocate the result slices (`results := make([][]byte, mapperCount)`) and assign values directly by index (`results[idx] = res`). This eliminates the need for mutexes entirely, prevents data races, and makes the resulting order deterministic. Additionally, when flattening slices, pre-calculate the total capacity needed (`make([]byte, 0, totalLen)`) before appending to reduce memory reallocations.
