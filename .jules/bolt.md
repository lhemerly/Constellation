...
## 2026-06-09 - Optimize Memory Allocation in MapReduceNode Flatten Phase
**Learning:** In `MapReduceNode`'s `mapReduceProcess`, repeatedly using `append` inside a loop to flatten `[][]byte` into a single `[]byte` without pre-allocating the capacity causes excessive memory allocations and slice copies, especially as the number of mappers increases. This overhead adds unnecessary latency during the reduction phase.
**Action:** Always pre-calculate the total length needed for flattened slices and use `make([]sliceType, 0, totalLen)` before iterating and appending. This eliminates redundant dynamic reallocations during concatenation.
