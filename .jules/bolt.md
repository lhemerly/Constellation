...
## 2024-06-15 - Optimize CacheMiddleware cache key calculation
**Learning:** Using `sha256.New()`, `hasher.Write()`, and `hex.EncodeToString(hasher.Sum(nil))` to generate string cache keys creates unnecessary memory allocations on the hot path of caching.
**Action:** Replaced dynamic string allocation with the fixed-size array `[32]byte` output of `sha256.Sum256(input)`. This eliminates several heap allocations, dropping allocation overhead to 0 and reducing cache key generation time.
