...
## 2024-05-30 - Optimize MapReduceNode Result Aggregation
**Learning:** Using `sync.Mutex` and `append` to collect results and errors from multiple concurrent goroutines in `MapReduceNode` introduces lock contention and repeated memory reallocations. Additionally, flattening the results into a single byte slice with repeated `append` operations without pre-allocating the total capacity caused further reallocations and GC pressure.
**Action:** When aggregating results from a known number of concurrent goroutines, pre-allocate the results slice (`make([]T, count)`) and assign values by index to eliminate the need for a mutex. When flattening slices, calculate the total length first and pre-allocate the destination slice (`make([]byte, 0, totalLen)`) to avoid reallocation overhead during `append`.
