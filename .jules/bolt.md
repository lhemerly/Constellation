...
## 2026-05-20 - MapReduceNode Slice Pre-allocation Optimization
**Learning:** During the reduction phase of `MapReduceNode.mapReduceProcess()`, dynamically appending to a shared slice under `sync.Mutex` during the mapping phase, and repeatedly appending dynamically to flatten the results during the reduction phase causes a high number of memory allocations and degrades performance due to frequent resizing and lock contention.
**Action:** Always pre-allocate slices when the size is known or can be calculated (e.g., `make([][]byte, len(mr.mappers))`), and use index-based assignment instead of `sync.Mutex` and `append`. For flattening multiple slices, pre-calculate the total combined length and pre-allocate the target slice (`make([]byte, 0, totalLen)`) before appending to eliminate repeated reallocation and copying.
