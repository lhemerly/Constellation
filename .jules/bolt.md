...
## 2024-05-25 - Optimize Hash Generation in CacheMiddleware
**Learning:** Using `sha256.New()`, `Write()`, and `Sum(nil)` followed by `hex.EncodeToString()` to generate cache keys in high-frequency paths (like `CacheMiddleware`) creates significant allocation overhead (5 allocs/op, ~1050 ns/op) due to intermediate objects and string allocation.
**Action:** Prefer `sha256.Sum256(input)` directly to generate a `[32]byte` hash, and use this fixed-size array directly as a map key instead of converting it to a string. This eliminates allocation overhead (reduces to 1 alloc/op, ~680 ns/op) and improves execution speed in hot paths.
