...
## 2024-04-24 - Optimize MapReduceNode slice allocation and lock contention
**Learning:** Appending to a shared slice across multiple goroutines using a `sync.Mutex` causes unnecessary lock contention on the happy path. Furthermore, appending `[]byte` sequentially to an unallocated array creates significant intermediate allocations.
**Action:** Always pre-allocate the slice capacity when the final size is known. When fanning-out work, preallocate the results slice (`make([][]byte, len)`) and insert items by their goroutine's index, making the assignment lock-free and deterministic. Precalculate the final required capacity for the aggregated byte slice to avoid incremental append allocations.
