...
## 2024-05-19 - Optimize Lock Contention and Memory Allocation in MapReduceNode
**Learning:** In `MapReduceNode.mapReduceProcess`, using a `sync.Mutex` inside the `for _, mapper := range mr.mappers` loop and `append`ing to `results` and `errs` slices dynamically created a lot of lock contention and memory reallocations. This was significantly slowing down the benchmark (~4504 ns/op vs ~3836 ns/op).
**Action:** Replaced the `sync.Mutex` and dynamic `append`s with pre-allocated slices based on `len(mr.mappers)`. Inside the goroutine, values are assigned by index (`results[idx] = res` and `errs[idx] = err`). This completely removes lock contention and optimizes memory allocations during map execution. Also flattened the reduce buffer with pre-calculated `make([]byte, 0, totalLen)`.
