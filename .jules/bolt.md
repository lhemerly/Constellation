...
## 2024-05-06 - [Concurrency in Node Teardown]
**Learning:** WaitGroup synchronization is crucial when testing and tearing down background processes, especially when dealing with timer callbacks like in `BatchingNode` where `Delete()` runs concurrently with `flushTimeout`.
**Action:** Always use WaitGroups for internal background work and invoke `wg.Wait()` before completing deletion logic to ensure clean teardowns and no leaked goroutines or timers.
