...
## 2024-05-15 - Optimize MapReduceNode Result Aggregation
**Learning:** Using `sync.Mutex` and `append` to aggregate results from concurrently running goroutines into a shared slice causes unnecessary lock contention and potential non-deterministic ordering. Furthermore, appending these slices together in a secondary loop without pre-allocating the final capacity leads to multiple expensive memory reallocations.
**Action:** When aggregating results from multiple goroutines where the count is known, pre-allocate the result slice (`make([]T, count)`) and allow goroutines to safely write their results directly to their corresponding index (`results[idx] = res`) without requiring a lock. Also, always pre-calculate total capacity before concatenating slices (`make([]byte, 0, totalLen)`) to eliminate dynamic memory allocations.
