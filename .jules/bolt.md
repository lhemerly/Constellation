...
## 2026-05-01 - Optimize MapReduceNode Map/Reduce Phases
**Learning:** Concurrent goroutine aggregation using `sync.Mutex` with `append` can be a performance bottleneck when the size and number of elements are known.
**Action:** When aggregating results from multiple concurrent goroutines, pre-allocate slices based on the known count and assign values by index instead of using `sync.Mutex` and `append`. For concatenation and flattening phases, calculate the exact length of the final output beforehand and initialize the slice capacity correctly with `make([]byte, 0, totalLen)` to prevent allocation overhead.
