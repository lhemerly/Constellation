...
## 2024-06-07 - Optimize mapReduceProcess Flattening Array Allocations
**Learning:** In the MapReduceNode's reduce phase, iteratively appending byte slices using `reducedInput = append(reducedInput, res...)` without pre-calculating the final array capacity results in excessive memory allocations. A benchmark with 100 mappers showed around 421 allocs/op and ~85,926 ns/op due to dynamic array resizing.
**Action:** When concatenating or flattening multiple byte slices, always calculate the total capacity first, then initialize the resulting array with `make([]byte, 0, totalLen)`. This prevents repeated reallocations, reducing memory overhead significantly.
