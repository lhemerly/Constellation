...
## 2026-06-08 - CacheMiddleware Key Generation Allocation Bottleneck
**Learning:** Using `sha256.New()` and `hex.EncodeToString()` for cache key generation inside the `CacheMiddleware` creates significant allocation overhead per request, because `sha256.New()` returns an interface forcing heap allocation, and hex encoding allocates strings. Using `[32]byte` arrays natively as map keys is supported in Go and drastically reduces allocations and latency.
**Action:** Always prefer `sha256.Sum256` which returns a stack-allocated `[32]byte` over `sha256.New` when a direct slice hash is needed, and use the `[32]byte` directly as a map key instead of converting it to a string when building high-performance caches.
