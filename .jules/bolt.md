...
## 2024-06-25 - Optimize Lock Contention in MapReduceNode
**Learning:** In `MapReduceNode.mapReduceProcess`, using a `sync.Mutex` to safely append mapping results to a shared slice caused high lock contention, especially with a large number of concurrent mappers. Using `append` under a lock essentially serializes the write operations.
**Action:** Pre-allocate result and error slices based on the known number of mappers, and assign values by their exact index concurrently (`results[i] = res`). This eliminates the need for a `sync.Mutex` lock entirely and ensures deterministic ordering. Apply this pattern whenever aggregating results from parallel workers with a known count.
