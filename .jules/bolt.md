...
## 2026-05-16 - Optimize Lock Contention and Memory Reallocations in MapReduceNode
**Learning:** Using `sync.Mutex` inside the inner loop of `MapReduceNode.mapReduceProcess` to append results to a shared slice caused high lock contention and non-deterministic result ordering during concurrent map phases. Additionally, dynamically appending mapped outputs during the reduce phase led to repeated memory reallocations. Benchmark showed ~138026 ns/op with 424 allocs/op before optimization.
**Action:** Replace `sync.Mutex` locks by pre-allocating the results slice based on the number of mappers and assigning values by index (`results[i] = res`). Use buffered channels to collect errors without locks. Furthermore, pre-calculate the total byte length of the results and pre-allocate the reduced byte slice before concatenating to prevent reallocation overhead. Post-optimization benchmark showed ~129713 ns/op with 405 allocs/op and deterministic order.
