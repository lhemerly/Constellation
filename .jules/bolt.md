...
## 2024-05-20 - Avoid allocations in cache key generation
**Learning:** Using `sha256.New()` followed by `.Write()` and `.Sum()` causes heap allocations when generating cache keys. Using the package-level `sha256.Sum256()` function operates completely on the stack, reducing allocations to 0 and dramatically improving execution speed in hot paths (like middlewares).
**Action:** Always prefer `sha256.Sum256()` when a one-shot hash of a contiguous byte slice is needed.
