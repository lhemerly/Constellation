...
## 2026-05-24 - Avoid RLock deadlocks during blocking channel operations
**Learning:** Holding a read lock (`RLock`) before a channel operation (e.g., sending to a worker queue in `Process`) causes deadlocks. If the buffer is full, the channel send blocks. If a shutdown method (e.g. `Delete()`) tries to acquire a `Lock` concurrently, it will hang indefinitely waiting for the read lock to be released, preventing cancellation.
**Action:** Remove explicit read locks protecting blocking channel sends. Rely entirely on `context.Context` for cancellation to safely escape the select statement, and do not explicitly close data channels in shutdown routines if doing so would require mutexes around sends to prevent panics.
