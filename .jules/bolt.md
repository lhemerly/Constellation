...
## 2026-05-16 - Prevent Data Races with Concurrent Workers and Timeouts
**Learning:** When using context timeouts () in a concurrent fan-out operation, writing directly to a shared slice from a worker goroutine and using a `select` block with a `done` channel in the parent can cause a data race. If the timeout occurs, the parent will proceed to read the shared slice while the worker might still be running and writing to it.
**Action:** Use a buffered channel to pass the full result (e.g., `make(chan struct{res []byte, err error}, 1)`) back to the parent instead of a simple `done` channel. The parent can select on the result channel or the timeout, keeping shared slices localized and thread-safe.
## 2026-05-16 - Prevent Data Races with Concurrent Workers and Timeouts
**Learning:** When using context timeouts in a concurrent fan-out operation, writing directly to a shared slice from a worker goroutine and using a select block with a done channel in the parent can cause a data race. If the timeout occurs, the parent will proceed to read the shared slice while the worker might still be running and writing to it.
**Action:** Use a buffered channel to pass the full result back to the parent instead of a simple done channel. The parent can select on the result channel or the timeout, keeping shared slices localized and thread-safe.
