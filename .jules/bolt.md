...
**Learning:** Using `sync.Mutex` with `append` to collect results from a known number of concurrent goroutines creates unnecessary lock contention. Similarly, concatenating multiple slices via repeated `append` without pre-allocating the target capacity causes multiple memory reallocations. Benchmarking showed an improvement of ~10% in execution speed and fewer allocations.
**Action:** When the number of concurrent operations is known, pre-allocate slices and use index-based assignment instead of `sync.Mutex` and `append`. When flattening multiple slices, compute the total length first and initialize the resulting slice with `make([]byte, 0, totalLen)`.
