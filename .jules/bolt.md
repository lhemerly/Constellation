...
## 2024-06-25 - Avoid heap allocations and timer leaks in performance-critical paths
**Learning:** Instantiating new objects (like `sha256.New()`) and converting byte slices to strings (`hex.EncodeToString()`) in frequently called middleware leads to unnecessary heap allocations. Using `[32]byte` arrays with `sha256.Sum256()` directly prevents this. Additionally, using `time.After()` in tight loops or rapidly executed middleware can leak timers, as they are only garbage collected when they expire. Using `time.NewTimer()` with a `defer timer.Stop()` ensures resources are freed immediately.
**Action:** Use fixed-size arrays for cache keys and avoid `hex.EncodeToString()` when possible. Always use `time.NewTimer` and immediately defer `timer.Stop()` when implementing timeouts in performance-critical paths instead of `time.After()`.
