...
## 2026-05-28 - Optimize CacheMiddleware key generation
**Learning:** Using `sha256.New()` followed by `Write()`, `Sum()`, and `hex.EncodeToString()` creates unnecessary heap allocations and string formatting overhead for high-frequency map keys. The benchmark showed ~755ns/op with this approach.
**Action:** Replace dynamic hashing and string encoding with `sha256.Sum256(input)` and use the resulting fixed-size `[32]byte` array directly as the map key. This approach is much faster (benchmark dropped to ~470ns/op) and avoids string allocation overhead while remaining completely safe since fixed-size byte arrays are valid and comparable map keys in Go.
