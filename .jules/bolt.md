...
## 2024-06-20 - Optimize Cache Key Allocation
**Learning:** Instantiating `sha256.New()` and calling `hex.EncodeToString()` to generate a string-based cache key from byte slices causes unnecessary heap allocations on every cache lookup.
**Action:** Use `sha256.Sum256(input)` directly and use the resulting fixed-size array `[32]byte` as the map key. This approach is allocation-free and measurably faster for high-throughput cache middleware.
