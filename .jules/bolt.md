...
## 2024-05-15 - Optimize BaseNode.Notify() to prevent lock contention
**Learning:** Holding a read lock (`mutex.RLock()`) for the entire duration of iterating through a map and waiting for concurrent spawned goroutines (`wg.Wait()`) inside `BaseNode.Notify()` significantly blocked writers (`Subscribe` and `Unsubscribe`), causing high lock contention and slowing down concurrent scenarios.
**Action:** Changed the approach to capture a snapshot of the subscribed nodes into a local slice under the `RLock`, quickly released the lock via `RUnlock()`, and then iterated over the local slice for executing the long-running notification operations. The benchmark for concurrent subscription and notifications improved dramatically, cutting execution time and avoiding stalling writers. When working with locking and iterating over potentially slow operations, prioritize capturing the data and releasing the lock immediately before execution.
