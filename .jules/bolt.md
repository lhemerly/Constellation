...
## 2024-06-29 - Optimize Cache Key Generation
**Learning:** Generating cache keys by instantiating `sha256.New()`, hashing bytes, and converting the sum to a hex string with `hex.EncodeToString` causes unnecessary heap allocations. Using `sha256.Sum256(input)` directly and using the resulting `[32]byte` array as a map key eliminates these allocations and improves execution time. Benchmark showed memory allocations dropped from 5 allocs/op to 1 allocs/op and time improved from ~4583 ns/op to ~4365 ns/op.
**Action:** When generating cache keys or hashing byte slices in performance-critical paths, prefer using `sha256.Sum256(input)` directly and use the fixed-size array `[32]byte` as a map key instead of instantiating `sha256.New()` and encoding strings with `hex.EncodeToString()`. This eliminates unnecessary heap allocations.
