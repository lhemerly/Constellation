...
## 2024-05-19 - Optimize Result Aggregation in MapReduceNode
**Learning:** Appending to a slice protected by a `sync.Mutex` inside a high-concurrency map-reduce fan-out creates lock contention and non-deterministic result ordering. In addition, repetitively appending to an unallocated byte slice (`var reducedInput []byte`) during the reduce phase leads to multiple memory reallocations.
**Action:** Always pre-allocate the results slice based on the number of workers/mappers and assign values by index concurrently. For byte slice flattening, pre-calculate the total target capacity by iterating over the results, allocate the slice using `make([]byte, 0, totalLen)`, and then perform the appends to eliminate unnecessary allocations.
