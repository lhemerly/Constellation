...
**Learning:** Using `sha256.New()`, `Write()`, and `hex.EncodeToString(hasher.Sum(nil))` to generate cache keys in high-frequency middleware creates unnecessary memory allocations and string conversion overhead. Benchmarks showed the old method took ~556ns/op while the new method took ~327ns/op.
**Action:** Use `sha256.Sum256(input)` directly and use the resulting `[32]byte` array as the map key instead of a string. Go's map implementation is highly optimized for array keys, avoiding heap allocations for both the hasher and the string. Apply this pattern anywhere SHA-256 is used for internal caching or lookup keys.
