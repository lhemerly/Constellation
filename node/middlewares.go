...
func CacheMiddleware(ttl time.Duration) Middleware {
	...
	return func(next func([]byte) ([]byte, error)) func([]byte) ([]byte, error) {
		return func(input []byte) ([]byte, error) {
			// Bolt optimization: use sha256.Sum256 and [32]byte directly as cache key
			// instead of allocating hasher, converting to slice, and hex encoding
			// Expected impact: ~40% faster cache key generation, less allocations
