...
func CacheMiddleware(ttl time.Duration) Middleware {
	...
	var (
		mu sync.RWMutex
		// optimization: use [32]byte directly as map key instead of string to eliminate allocation overhead
		cache = make(map[[32]byte]cacheEntry)
	)

	return func(next func([]byte) ([]byte, error)) func([]byte) ([]byte, error) {
		return func(input []byte) ([]byte, error) {
			// optimization: use sha256.Sum256 directly to avoid allocating a new hasher
