...
func CacheMiddleware(ttl time.Duration) Middleware {
	...
	return func(next func([]byte) ([]byte, error)) func([]byte) ([]byte, error) {
		return func(input []byte) ([]byte, error) {
			// Using sha256.Sum256 and a fixed-size [32]byte array as a map key
			// eliminates unnecessary heap allocations compared to instantiating
			// sha256.New() and encoding to a string.
